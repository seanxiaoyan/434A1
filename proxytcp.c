...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        if((buf_len = recv(clientfd,buf_proxy,SIZE,0)) == -1){
            printf("recv() error\n");
            exit(1);
        }
        ...
        if (strcmp(buf_proxy,"all")==0){
            ...
            send(serverfd,"Monday",6,0);
            if((buf_len = recv(serverfd,buf_server,SIZE,0)) == -1){
                printf("recv() error\n");
                exit(1);
            }
            ...
            close(serverfd);
            memset(buf_server,0,SIZE);
            send(clientfd,weather_all,strlen(weather_all),0);
